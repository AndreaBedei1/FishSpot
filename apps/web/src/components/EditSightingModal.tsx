...
type Species = { id: number; name: string; scientificName?: string };

export default function EditSightingModal({ open, onClose, sighting, onUpdated }: Props) {
  const [formData, setFormData] = useState({
    specimens: sighting.specimens.toString(),
    latitude: sighting.latitude.toString(),
    longitude: sighting.longitude.toString(),
    animalId: sighting.animal?.id?.toString() || "",
    speciesId: sighting.species?.id?.toString() || "",
    sea: sighting.sea || "",
    wind: sighting.wind || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Errore durante l’aggiornamento dell’avvistamento");
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg mx-auto overflow-y-auto max-h-[90vh] relative z-[10000]">
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              value={new Date(sighting.date).toLocaleDateString()}
              disabled
              className="w-full border rounded-lg px-3 py-2 bg-gray-100 cursor-not-allowed"
            />
          </div>

          {/* Utente (non modificabile) */}
          <div>
          ...
          {/* Numero esemplari */}
          <div>
            <label className="block text-sm font-medium mb-1">Num. esemplari*</label>
            <input
              type="number"
              name="specimens"
